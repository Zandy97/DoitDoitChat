...
SELECT cr.ROOM_ID, cr.ROOM_NAME, CHAT_TIME, CHAT_ID, EMP_ID
	FROM CHAT_ROOM cr,
		JSON_TABLE(ROOM_MEM, '$.ROOM[*]'
		COLUMNS (
			 IDS VARCHAR(20) PATH '$.id'
...
SELECT ROOM_ID,CHAT_TIME
FROM( 
SELECT ROOM_ID,CHAT_TIME ,ROW_NUMBER () OVER(PARTITION BY ROOM_ID ORDER BY CHAT_TIME DESC) AS mx
	FROM CHAT c 
)
WHERE mx = 1
...
ALTER TABLE CHAT ADD CHAT_TYPE CHAR(1);
